...
-- Attendu : 8 focaccias (Americaine -> Tradizione)
-- Obtenu  : Conforme
...
-- Attendu : 8 lignes (Paysanne a le max avec 12)
-- Obtenu  : Conforme
...
HAVING COUNT(c.id_ingredient) = (
    SELECT MAX(nb) 
    FROM (SELECT COUNT(id_ingredient) AS nb FROM comprend GROUP BY id_focaccia) t
);
